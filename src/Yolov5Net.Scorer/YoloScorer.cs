...
namespace Yolov5Net.Scorer
{
    ...
    public class YoloScorer<T> : IDisposable where T : YoloModel
    {
        ...
        /// </summary>
        public YoloScorer(T model, string weights, SessionOptions opts = null) : this()
        {
            _model = model;
        ...
        public YoloScorer(Stream weights, SessionOptions opts = null) : this()
        {
            ...
            {
                _inferenceSession =
                    new InferenceSession(reader.ReadBytes((int)weights.Length), opts ?? new SessionOptions());
            }
        }
        ...
        private DenseTensor<float>[] Inference(Image image)
        {
            ...
            if (image.Width != _model.Width ||
                image.Height != _model.Height) resized = ResizeImage(image); // fit image size to specified input size
                ...
                Parallel.For(0, _model.Anchors[0].Length, a => // iterate anchors
                {
                    Parallel.For(0, shapes, y => // iterate shapes (rows)
                    {
                        Parallel.For(0, shapes, x => // iterate shapes (columns)
                        {
                            ...
                            var xMin = Clamp((xyxy[0] - xPad) / gain, 0, w - 0); // unpad, clip tlx
                            var yMin = Clamp((xyxy[1] - yPad) / gain, 0, h - 0); // unpad, clip tly
                            var xMax = Clamp((xyxy[2] - xPad) / gain, 0, w - 1); // unpad, clip brx
                            var yMax = Clamp((xyxy[3] - yPad) / gain, 0, h - 1); // unpad, clip bry
